...
$gamePath = if ($env:PEAK_GAME_PATH -and (Test-Path $env:PEAK_GAME_PATH)) {
    $env:PEAK_GAME_PATH
} elseif (Test-Path 'C:\Program Files (x86)\Steam\steamapps\common\Peak') {
    ...
    Write-Host 'Set PEAK_GAME_PATH environment variable' -ForegroundColor Yellow
    exit 1
}
