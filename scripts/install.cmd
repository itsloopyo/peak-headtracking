...
set "GAME_EXE=Peak.exe"
set "GAME_DISPLAY_NAME=PEAK"
set "STEAM_FOLDER_NAME=PEAK"
set "ENV_VAR_NAME=PEAK_PATH"
...
set "BEPINEX_VERSION=5.4.75301"
set "BEPINEX_ARCH=x64"
set "BEPINEX_URL=https://thunderstore.io/package/download/BepInEx/BepInExPack_PEAK/5.4.75301/"
...
:: ============================================
:install_bepinex
if defined BEPINEX_URL (
    set "BEP_URL=%BEPINEX_URL%"
) else (
...
if defined BEPINEX_SUBFOLDER (
    :: Thunderstore packs have a subfolder — extract to temp, then copy contents
    if exist "%BEP_TEMP%" rmdir /s /q "%BEP_TEMP%"
    mkdir "%BEP_TEMP%"
    tar -xf "%BEP_ZIP%" -C "%BEP_TEMP%"
    if errorlevel 1 (
...
:: Don't overwrite BepInEx.cfg if it already exists (Thunderstore pack ships preconfigured)
